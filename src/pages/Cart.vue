<script setup>
  ...
  // const cartItem = computed(() => store.state.cart.cart);
  const cartItem = computed(() => store.getters["cart/allItems"]);
  const cartTotalAmount = computed(() => store.getters["cart/cartTotalAmount"]);
...
<template>
  <div class="container">
    <div class="row mt-5">
      <div v-if="cartItem.length == 0" class="col-md-12 text-center">
        <div>
          <i class="bi bi-basket-fill" style="font-size: 100px"></i>
        </div>
        <h3 class="text-bold">Cart is empty</h3>
        <router-link :to="{name: 'product'}" class="btn btn-outline-dark mt-3">Products</router-link>
      </div>
          ...
          <tbody>
            <tr v-for="item in cartItem" :key="item.id">
              ...
              <td class="align-middle">
                ...
                <button @click="decrement(item.id)" class="btn btn-sm btn-dark me-2">-</button>
              </td>
